...
- 控制台提供管理 Agent 配置与执行顺序的独立页面。用户可逐条新增、删除和上下调整 Codex 或 Claude Code 关联项；每条可编辑 Agent、模型名、可选 Base URL 和可选 API Key，同一 Agent 可配置多次。
- 模型配置按页面顺序写入数据库相邻的 `console-settings.json`。第一条是默认配置；启用自动切换时，仅在额度耗尽或鉴权失败后依次尝试后续配置，每次尝试必须使用该条自己的模型、Base URL 和密钥。
