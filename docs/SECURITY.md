...
- Agent 会继承服务进程环境并使用对应 Codex 或 Claude Code CLI 的已有模型、服务地址和凭据。Web 模型设置只保存 Agent 类型与执行顺序，不编辑或保存模型名、Base URL 和 API Key；任务输出仍可能包含 Agent 或工具主动打印的敏感信息。
...
- Codex / Claude Code 登录在服务进程用户下执行，生成的长期凭据由各自官方 CLI 写入用户凭据目录，不写入项目配置。Docker 部署会把这些目录映射到宿主机 `data/codex` 与 `data/claude` 以跨容器重建保留登录，两者都必须按密钥材料保护。一次性设备码、callback 输入和登录输出会经无缓存控制台 API 返回或短暂保存在服务内存中，因此控制台访问者等同于能够发起或观察该用户的 Agent 授权；必须通过可信网络和访问控制保护。不要记录或转发 OAuth callback 完整地址。
- 未选择创建 Pull Request 时不执行提交和推送。选择后系统会暂存工作区全部改动并使用当前 Git/`gh` 身份推送，因此必须在提交前审查目标仓库、提示和运行日志。
- 钉钉和飞书的 `开发` 命令默认创建 Pull Request，并使用当前配置的 GitHub Token 与 Agent CLI 身份。消息任务只允许创建它的同一平台、同一用户查询或取消，但当前平台客户端把结果推送到配置的固定群机器人 Webhook，不提供多群会话级路由；应为每个可信群单独部署或配置实例。
