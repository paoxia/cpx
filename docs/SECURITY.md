...
- Agent 会继承服务进程环境。模型页面可保存独立 API Key；密钥会以明文写入 `console-settings.json`，并只把当前尝试项的密钥注入对应 Agent 子进程。设置 API 只返回是否已配置和密钥来源，不返回原文；浏览器中的密码输入框也不会回填已存密钥。该文件已被 Git 忽略，但仍需使用操作系统权限限制读取。任务输出仍可能包含 Agent 或工具主动打印的敏感信息。
- 每条模型配置可指定自定义 Base URL，任务提示、仓库代码和模型请求会发送到该地址代表的服务。仅应使用可信、受控的 HTTPS 网关；系统会拒绝包含凭据、片段或非 HTTP(S) 协议的地址，但不会验证远端运营方。
- 独立模型测试弹窗会让用户从当前关联模型中选择一项，并把页面当前的模型、Base URL 和临时或已存密钥注入一次短生命周期 CLI 子进程，产生一次真实模型请求。用户输入和提取后的 Agent 文本回复会通过无缓存 API 返回，但不会持久化；回复限制为 16 KiB，且会替换当前配置中可识别的密钥。不要在测试内容中提交无关敏感数据，也不要假定模型回复天然可信。
