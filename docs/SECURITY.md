...
- Agent 会继承服务进程环境。模型管理页输入的 API Key 会以明文写入 `console-settings.json`，并只把当前尝试项的密钥注入对应 Agent 子进程；设置 API 只返回是否已配置和密钥来源，不返回原文。该文件已被 Git 忽略，但仍需使用操作系统权限限制读取。任务输出仍可能包含 Agent 或工具主动打印的敏感信息。
