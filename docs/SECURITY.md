...
- Agent 会继承服务进程环境。设置 API 兼容的独立 API Key 会以明文写入 `console-settings.json`，并只把当前尝试项的密钥注入对应 Agent 子进程；模型页面不显示或修改 API Key，设置 API 也只返回是否已配置和密钥来源，不返回原文。该文件已被 Git 忽略，但仍需使用操作系统权限限制读取。任务输出仍可能包含 Agent 或工具主动打印的敏感信息。
- 独立模型测试弹窗会让用户从当前关联模型中选择一项，并把服务端已有密钥注入一次短生命周期 CLI 子进程，产生一次真实模型请求。测试接口不保存页面临时值、不返回完整模型输出，并对错误摘要中的当前密钥做替换脱敏。
