...
控制台将有序 Agent 关联项持久化到数据库相邻的 `console-settings.json`，每项只包含 id 与 provider。模型页面只编辑 Codex/Claude Code 类型与执行顺序；模型名、Base URL 和 API Key 完全沿用对应 CLI 或服务进程环境，加载旧设置时会迁移到版本 4 并删除这些历史覆盖字段。每条关联项内提供测试区；`ModelConfigurationTester` 使用对应 CLI 的已有配置发送用户内容，从 Codex 或 Claude Code JSONL 事件中提取最终文本回复，并在长度限制后返回原关联项下方。测试内容和回复不持久化。创建任务时，控制台可通过 GitHub API 分页读取所选仓库分支；选择现有分支后，`AgentTaskManager` 以它为浅克隆基线并创建自动 `cpx/task-<id>` 分支，选择新建分支时则使用用户提供的合法名称，Pull Request 明确以所选基础分支为 base。Agent 按关联项顺序启动 CLI，同一 provider 可以在顺序中出现多次。未选择创建 PR 时，改动只保留在本地工作区；选择后才执行提交、推送和 `gh pr create`。任务及最近 800 条日志保存在进程内，工作区保存在磁盘。
